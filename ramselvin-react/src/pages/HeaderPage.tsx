...
import { ROUTE_INFORMATION, ROUTE_REPOSITORY, ROUTE_LOGIN, ROUTE_DATABASE } from '@/constants/route';
import { LOGO_MASTER } from '@/constants/resource';
import { REPOSITORY, INFORMATION, NAME_APP, LOGOUT, DATABASE } from '@/constants/components';
        ...
        <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
            <div className="container flex h-14 max-w-screen-2xl items-center">
                <div className="mr-4 hidden md:flex">
                    ...
                    <nav className="flex items-center gap-4 text-sm lg:gap-6">
                        {isAuthenticated ? (
                            <>
                                <Link className="transition-colors hover:text-foreground/80 text-foreground" to={ROUTE_DATABASE}>{DATABASE}</Link>
            ...
            {isMenuOpen && (
                <div className="p-6 md:hidden">
                    <nav>
                        <ul>
                            <li><Link to={ROUTE_DATABASE}>{DATABASE}</Link></li>
